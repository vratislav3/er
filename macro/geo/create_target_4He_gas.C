...
void create_target_4He_gas() {
  ...
  TString geoFileName = geoPath + "/geometry/target.4He.gas.root";
  // --------------------------------------------------------------------------
  ...
  FairGeoMedium* m4He = geoMedia->getMedium("helium");
  if ( ! m4He ) Fatal("SOS", "FairMedium helium not found");
  geoBuild->createMedium(m4He);
  TGeoMedium* p4He = gGeoMan->GetMedium("helium");
  if ( ! p4He ) Fatal("SOS", "Medium helium not found"); 
  ...
  TGeoVolume* tubeD2 = new TGeoVolume("tubeD2", D2_shape, p4He);
